...
public class Maj_admin_prof_ajouter extends JFrame implements ActionListener
{
    ...
    public Maj_admin_prof_ajouter()
    {
        ...
        this.case_IDdiscipline.addActionListener(this);
    ...
    { 
        ...
        try{
            ...
            String get5=case_IDdiscipline.getText().trim();
            
            if((get.equals(""))||(get2.equals(""))||(get3.equals(""))||(get4.equals(""))||(get5.equals("")))
            {
                ...
                else
            {
                 ...
                 int id3=Integer.parseInt(get5);
                ajout_enseignant(id2,id1,id3,get3,get4);
            }
      ...
      /**
  * Ajout d'un enseignant à partir des des parametres renseignés:
     * @param id_classe classe dans laquel il enseigne
     * @param id_discipline discipline qu'il enseigne
     * @param id_enseignant id du prof
     * @param nom son nom
     * @param prenom son prenom
     * @return  1 si c bon, 2 si l'enseignent existe deja*/
    
    public int ajout_enseignant(int id_enseignant,int id_classe,int id_discipline, String nom, String prenom)
{   
    Enseignant enseignant=new Enseignant(id_enseignant,nom, prenom);
        
    Enseignement enseignement=new Enseignement(id_discipline,id_enseignant,id_classe);
    
    try{
    Connexion co= new Connexion("java2019","root","");
    DAO<Enseignant> e=new DAO_Enseignant(co);
    DAO<Classe> c=new DAO_Classe(co);
    DAO<Discipline> d=new DAO_Discipline(co);
    DAO<Enseignement> en=new DAO_Enseignement(co);
    int test=0;
    int test2=0;
    int test3=0;
    
        for(int i=0;i<e.getids().size();i++) // on cherche si l'enseignant n'existe pas déja
    {
    ...
    for(int i=0;i<c.getids().size();i++) // on cherche si la classe existe 
    {
    ...
    for(int i=0;i<d.getids().size();i++) // on cherche si la discipline affecté existe existe 
    {
        if( Integer.parseInt(d.getids().get(i).trim())== id_discipline) 
        {
    ...
    if (test==0 & test2==1 & test3==1) // si l'enseignant n'existe pas deja, que la classe existe et que la discipline existe
    {
        e.create(enseignant); 
        en.create(enseignement);
        ...
        if(test==1) JOptionPane.showMessageDialog(jPanel1, "Ajout echoué, l'enseignant existe déja!");
        if(test2==0) JOptionPane.showMessageDialog(jPanel1, "Ajout echoué, la classe n'existe pas!");
        if(test3==0) JOptionPane.showMessageDialog(jPanel1, "Ajout echoué, la discipline n'existe pas!");
    return 2;
    }
